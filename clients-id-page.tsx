                ...
                <select name="course" required className={inputClass}>
                  {courses.map((c) => (
                    <option key={c.id} value={c.courseName}>
                      {c.courseName} (R$ {c.price.toLocaleString("pt-BR")} ·
                      comissão R$ {c.commission.toLocaleString("pt-BR")})
                    </option>
                  ))}
                </select>
              </div>
              <FormField name="monthYear" label="Mês" required />
