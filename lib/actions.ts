...
export async function createClient(formData: FormData) {
  ...
  await prisma.client.create({
    data: {
      ...
      monthYear: String(formData.get("monthYear")),
      ...
      entryPayment: String(
        formData.get("entryPayment")
      ) as "PIX" | "CARTAO",
      remainderPayment: String(formData.get("remainderPayment")) as
        | "PIX"
        | "CARTAO_PARCELADO"
        | "BOLETO_PARCELADO",
      installments: formData.get("installments")
        ...
        : null,
      boletoDueDay: formData.get("boletoDueDay")
...
export async function updateClientStatus(clientId: string, status: string) {
  ...
  await prisma.client.update({
    where: { id: clientId },
    data: {
      status: status as "INICIANDO" | "INTERMEDIARIO" | "FECHANDO",
    },
  });
...
export async function cancelSale(saleId: string) {
  ...
  if (sale.sellerId !== user.id && user.role !== "ADMIN") {
    throw new Error("Sem permissão para cancelar esta venda.");
  }
  if (sale.cancelledAt) return; // já cancelada
